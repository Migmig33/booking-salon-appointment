...
export default function ServicesPage({ navigate, startBooking }: Props) {
  ...
  const visibleGroups = SERVICE_GROUPS.map((group) => ({
    ...
    services: services.filter(
      (service) =>
        (group.categories as readonly string[]).includes(service.category) &&
    ...
    <main className="pb-20 lg:pb-0">
      ...
      <section className="py-16 lg:py-20 bg-cream border-b border-warm-line">
        <div className="max-w-7xl mx-auto px-5 lg:px-8">
          ...
          <h1 className="font-serif text-[40px] lg:text-[52px] text-charcoal mt-3 mb-4">
            Services
          </h1>
          ...
          {loading && (
            ...
            >
              {Array.from({ length: 8 }).map((_, index) => (
                <div key={index} className="h-64 bg-cream-dark animate-pulse" />
              ))}
            </div>
          )}
          {!loading && error && (
            ...
            visibleGroups.map((group) => (
              <div key={group.title}>
                <div className="flex items-center gap-4 mb-7 lg:mb-8">
                  <h2 className="font-serif text-[24px] lg:text-[28px] text-charcoal">
                    {group.title}
                  </h2>
                  ...
                  {group.services.map((service) => (
                    ...
                    >
                      <div className="flex-1">
                        <h3 className="font-serif text-[17px] text-charcoal mb-2">
                          {service.name}
                        </h3>
      ...
      <section className="py-16 lg:py-20 bg-charcoal text-center">
        <div className="max-w-xl mx-auto px-5">
          ...
          <p className="text-[14px] text-cream/65 leading-relaxed mb-8">
            Book a consultation or email the demo contact to explore how this
            flow works for a salon.
          </p>
